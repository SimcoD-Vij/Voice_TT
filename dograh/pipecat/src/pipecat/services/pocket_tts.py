...
class PocketTTSService(TTSService):
    ...
    - Resamples 24000 Hz float32 → 16000 Hz int16.
    """
            ...
            try:
                ...
                async with session.post(
                    ...
                    async for chunk in resp.content.iter_chunked(1200):
                        ...
                        # when user interrupts — no manual flag needed
                ...
                logger.info("[PocketTTS] ✅ Interrupted by user — synthesis cancelled")
                # DO NOT re-raise — let it fall through to TTSStoppedFrame
                ...
                # Always emit stop frame — pipeline needs this to clean up
                yield TTSStoppedFrame(context_id=context_id)
