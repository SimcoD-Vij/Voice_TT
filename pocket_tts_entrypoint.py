"""
pocket-tts entrypoint — decoupled startup:
  - Model loads first
  - Uvicorn starts in background  
  - Readiness written to /tmp/ready when model is injected
  - Healthcheck reads /tmp/ready (no network dependency during load)
"""
...
# This means the port is open and healthcheck won't get "connection refused"
# But requests will fail until model is injected — that's OK, they'll error not hang
