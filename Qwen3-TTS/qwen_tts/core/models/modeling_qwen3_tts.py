...
try:
    ...
    def create_causal_mask(config, input_embeds, attention_mask, cache_position, past_key_values=None, **kwargs):
        q_len = input_embeds.shape[1]
        kv_len = cache_position[-1] + 1 if cache_position is not None else q_len
        mask = torch.full((q_len, kv_len), torch.finfo(input_embeds.dtype).min, device=input_embeds.device)
        mask.triu_(diagonal=kv_len - q_len + 1)
        return mask[None, None, :, :].to(input_embeds.dtype)

    def create_sliding_window_causal_mask(config, input_embeds, attention_mask, cache_position, past_key_values=None, **kwargs):
        return create_causal_mask(config, input_embeds, attention_mask, cache_position, past_key_values)
...
class Qwen3TTSTalkerModel(Qwen3TTSTalkerTextPreTrainedModel):
    ...
    def __init__(self, config: Qwen3TTSConfig, *args, **kwargs):
...
class Qwen3TTSTalkerForConditionalGeneration(Qwen3TTSTalkerTextPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["codec_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}
    _pp_plan = {"lm_head": (["hidden_states"], ["logits"])}
    config_class = Qwen3TTSTalkerConfig
    base_model_prefix = "talker"

    def __init__(self, config: Qwen3TTSTalkerConfig, *args, **kwargs):
        super().__init__(config, *args, **kwargs)
